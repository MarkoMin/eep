...
****
EEP XX: Parametrized module type
----

Abstract
========

Although Erlang isn't statically typed language, it still can benefit a lot
from relying on type specifications. Currently, tools for type analysis don't
have a mechanism to anlyze dynamic function calls. This EEP introduces a new
type that enables more semantically valuable information about callback modules
to both humans and type analysis tools.

Motivation
==========

MFA calls where either `M` or `F` aren't atom literals are currently considered
as dynamic. Therefore we loose the ability to propagate type checking to those
calls. There isn't a way to specify any
